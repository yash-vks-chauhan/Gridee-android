...
struct MyVehiclesView: View {
    ...
    @State private var showingVehicleEditor = false
    @State private var vehicleEditorMode: VehicleEditorMode = .add
    @State private var editingIndex: Int? = nil
    @State private var showingDeleteAlert = false
    @State private var deleteIndex: Int? = nil
    
    enum VehicleEditorMode: Equatable {
        case add
        case edit(existing: VehicleData)
    }
...
struct VehicleRow: View {
    ...
    var body: some View {
        HStack(spacing: 16) {
            ZStack {
                RoundedRectangle(cornerRadius: 12)
                    .fill(vehicle.swiftUIColor.opacity(0.2))
                    .frame(width: 50, height: 50)
                
                Image(systemName: "car.fill")
                    .foregroundColor(vehicle.swiftUIColor)
            }
    ...
    var body: some View {
        VStack(spacing: 24) {
            ...
            VStack(spacing: 8) {
                ...
                Text("Add your vehicle details to quickly select them when booking.")
                    .font(.subheadline)
                    .foregroundColor(.secondary)
                    .multilineTextAlignment(.center)
                    .padding(.horizontal, 32)
...
// MARK: - Vehicle Editor Sheet
struct VehicleEditorSheet: View {
    let mode: MyVehiclesView.VehicleEditorMode
    let existingVehicles: [VehicleData]
    let onSave: (VehicleData) -> Void
    let onCancel: () -> Void
        ...
        NavigationView {
            Form {
                ...
                Section("Vehicle Name") {
                    TextField("e.g., Honda City", text: $vehicleName)
                }
                
                Section("Color") {
                    ColorPicker("Vehicle Color", selection: Binding(
                        get: { Color(hex: selectedColorHex) ?? .blue },
                        set: { selectedColorHex = $0.toHex() }
                    ))
                }
            }
            .navigationTitle(mode == .add ? "Add Vehicle" : "Edit Vehicle")
            .navigationBarTitleDisplayMode(.inline)
            .toolbar {
                ToolbarItem(placement: .navigationBarLeading) {
                    Button("Cancel") { onCancel() }
                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    ...
                    .disabled(registrationNumber.isEmpty || vehicleName.isEmpty)
                }
            }
            .onAppear {
                if case .edit(let existing) = mode {
