...
//        // Use authViewModel.currentUser, NOT $authViewModel.currentUser
...
//        let lastInitial = components.count > 1 ? components.last?.first ?? "S" : "S"
//        
...
struct ContentView: View {
    @StateObject private var authViewModel = AuthViewModel()
    @EnvironmentObject var themeManager: ThemeManager // ✅ CHANGED THIS LINE
                ...
                if homeViewModel.isSearching {
                    ...
                    if !homeViewModel.userBookings.isEmpty || homeViewModel.walletBalance > 0 {
                        UserStatsSection(
                            ...
                            pendingBookings: homeViewModel.pendingBookingsCount,
                            onActiveTap: {
                                // ✅ Navigate to Bookings tab and select Active (index 0)
                                selectedBookingTab = 0
                                selectedTab = .booking
                            },
                            onPendingTap: {
                                // ✅ Navigate to Bookings tab and select Pending (index 1)
