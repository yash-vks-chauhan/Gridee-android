...
struct RealTimeParkingCard: View {
    ...
    private var iconBackground: Color {
        colorScheme == .dark ? Color.gray.opacity(0.4) : .black
    }

    private var buttonBackground: Color {
        isBooking ? .gray : (colorScheme == .dark ? .white : .black)
    }
    ...
    // MARK: - Zone Name (BACKEND SOURCE OF TRUTH)
    private var displayZoneName: String {
        if let zone = parkingSpot.zoneName, !zone.isEmpty {
            return zone
        }

        if let capacity = parkingSpot.capacity {
            if capacity >= 500 { return "Large Parking Area" }
            if capacity >= 200 { return "Medium Parking Area" }
            if capacity > 0 { return "Small Parking Area" }
        }
    ...
    var body: some View {
        Button(action: isBooking ? {} : onBookTap) {
            HStack(spacing: 16) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    Text("₹\(parkingSpot.bookingRate)/hour")
                        .font(.system(size: 13))
                        .foregroundColor(secondaryTextColor)
                }
                ...
                Button(action: isBooking ? {} : onBookTap) {
                    if isBooking {
                        ProgressView()
                            .progressViewStyle(
                                CircularProgressViewStyle(tint: buttonTextColor)
                            )
        ...
        .padding(.vertical, 8)
        .onAppear {
            // MARK: Debug (safe to remove in production)
            print("🔍 ParkingSpot Card Loaded")
            print("   Spot ID: \(parkingSpot.id)")
            print("   Zone: \(parkingSpot.zoneName ?? "nil")")
            print("   Lot ID: \(parkingSpot.lotId ?? "nil")")
            print("   Available: \(parkingSpot.available ?? 0)")
            print("   Capacity: \(parkingSpot.capacity ?? 0)")
//            print("   Rate: ₹\(parkingSpot.bookingRate)")
            print("------------------------------------")
        }
    }
}
